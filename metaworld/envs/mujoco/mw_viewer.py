...
class MwViewer(mujoco_py.MjViewerBasic):
    """
    ...
    def _create_full_overlay(self):
        ...
        self.add_overlay(const.GRID_TOPLEFT, "Sawyer Arm", "WASD Up/Down Arrow")
        self.add_overlay(const.GRID_TOPLEFT, "Gripper Open/Close", "Left/Right Arrow")
        self.add_overlay(const.GRID_TOPLEFT, "[r]eset environment", "")
        self.add_overlay(const.GRID_TOPLEFT, "[n]ext task", "")
        for k, v in self._info.items():
            if type(v) == str:
                s = v
            else:
                s =  "{:.3f}".format(v)
            self.add_overlay(const.GRID_TOPLEFT, k, s)
    ...
    def key_action(self, key, action, forward, backward, action_idx):
        if key == forward:
            if action == glfw.PRESS:
                self._action[action_idx] = 1.0
            elif action == glfw.RELEASE:
                self._action[action_idx] = 0.0
        elif key == backward:
            if action == glfw.PRESS:
                self._action[action_idx] = -1.0
            elif action == glfw.RELEASE:
    ...
    def key_callback(self, window, key, scancode, action, mods):
        self.key_action(key, action, glfw.KEY_W, glfw.KEY_S, 1)
        self.key_action(key, action, glfw.KEY_D, glfw.KEY_A, 0)
        self.key_action(key, action, glfw.KEY_UP, glfw.KEY_DOWN, 2)
        self.key_action(key, action, glfw.KEY_RIGHT, glfw.KEY_LEFT, 3)
        if action != glfw.RELEASE:
            return
        elif key == glfw.KEY_R: # Reset environment
            self.reset_requested = True
        elif key == glfw.KEY_N: # New task
            self.newtask_requested = True
